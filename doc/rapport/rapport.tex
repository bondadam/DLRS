...
\begin{figure}[H]
        ...
        Parmi les 305 features, plus de la moitié étaient des coefficients d'une transformée de Fourier, et 48 parmi les 50 premières. Comme on peut le voir sur la table, les premières features et particulièrement la toute première, ont des p-value extrêmement basses, plusieurs ordres de grandeur plus petites que les suivantes. Les données complètes des \emph{features} retenues sont disponibles dans le fichier \texttt{tsfresh\_relevance\_features.csv}.
...
\end{figure}
    ...
    En revanche, nous pensons que les deux autres approches ont du potentiel pour résoudre le problème de l'entreprise. L'approche par série temporelle avec extraction de quelques features est plus efficace et rapide et donc adaptée à un système embarqué, mais l'approche par détection d'objets est plus originale, convient mieux au mode de fonctionnement actuel de l'entreprise, et peut identifier des pics que la première approche ne repèrerait pas.
...
\bibitem{sobel} \href{https://fr.wikipedia.org/wiki/Filtre\_de\_Sobel}{Page wikipédia sur le filtre de sobel, \texttt{https://fr.wikipedia.org/wiki/Filtre\_de\_Sobel}}
\end{thebibliography}
\endgroup
\clearpage
